...
public class SubqueriesTest extends EntityManagerTest {
	...
	public void clientesQueGastamMaisQue100Reais2ponto0() {
		...
		String jpql = "Select c from Cliente c " + "where " + "500 < (select sum(p.total) from Pedido p where "
				+ "p.cliente = c)";
	...
	public void subQuerieComExists() {
		...
		// POSSO USAR EXISTS E NOT EXISTS PENSAR NISSO NO STATUS!!! QUERO TODOS AS
		// SOLICITACOES
		// QUE TENHAM STATUS <= X E NUNCA STATUS > X
		// EXISTS TEM QUE COMPARAR O SELECT 1 COM O 2, MELHOR Q SEJA ASSIM!
		String jpql = "Select p from Produto p where exists " + "(select 1 from ItemPedido ip2 join ip2.produto p2 "
	...
	public void trazerPedidosQueContenhamProdutoDaCategoria2() {
		...
		String jpql = "Select p from Pedido p where p.id in " + "(select p2.id from ItemPedido i2 "
...
//				+ "p.cliente = c)";
	...
	public void produtosQueNaoForamVendidosComOPrecoAtual() {
		...
		String jpql2 = "select p from Produto p " + " where exists "
	...
	// @Test
	public void produtosQueFramVendidosPeloMenosUmaVezPeloPrecoAtual() {
	...
	// @Test
	public void produtosQueFramVendidosComPrecoDiferenteDoAtual() {
		// PODEMOS USAR COM ANY OU SOME, SAO IGUAIS!
		String jpql = "select p from Produto p where "
				+ "p.preco <> ANY (select precoProduto from ItemPedido where produto = p)";
	...
	@Test
	public void todosOsProdutosQueSempreForamVendidosPeloMesmoPreco() {

		// preco produto de itemPedido, usar distinct, join com produto
		// from pra itemPedido
		// o kindle nao pode voltar nessa consulta!!!! vai voltar o 3 e 4

		String jpql = "select distinct p from ItemPedido ip join ip.produto p where " + " ip.precoProduto = ALL "
